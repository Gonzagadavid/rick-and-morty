...
export const useMortyMove = ({ context, start, gameOver } : MortyMoveParams) => {
  ...
  const keyEvent = (event:KeyboardEvent<HTMLDivElement>) => {
    if (gameOver) return;
    if ((event.code === 'ArrowUp' || event.code === 'Numpad8' || event.code === 'KeyW') && MortyY - 30 > -40) {
      setup(true);
      setMortyY((prev) => prev - 30);
    }
    if ((event.code === 'ArrowDown' || event.code === 'Numpad2' || event.code === 'KeyS') && MortyY + 30 < 470) {
      setup(false);
      setMortyY((prev) => prev + 30);
    }
    if ((event.code === 'ArrowLeft' || event.code === 'Numpad4' || event.code === 'KeyA') && MortyX - 30 > -40) {
      setRight(false);
      setup(true);
      setMortyX((prev) => prev - 30);
    }
    if ((event.code === 'ArrowRight' || event.code === 'Numpad6' || event.code === 'KeyD') && MortyX + 30 < 1080) {
      setRight(true);
      setup(false);
      setMortyX((prev) => prev + 30);
    }
    if ((event.code === 'Numpad9' || event.code === 'KeyE') && MortyX + 30 < 800 && MortyY - 30 > -40) {
      setRight(true);
      setup(false);
      setMortyX((prev) => prev + 30);
      setMortyY((prev) => prev - 30);
    }
    if ((event.code === 'Numpad7' || event.code === 'KeyQ') && MortyX - 30 > -40 && MortyY - 30 > -40) {
      setRight(false);
      setup(false);
      setMortyX((prev) => prev - 30);
      setMortyY((prev) => prev - 30);
    }
    if ((event.code === 'Numpad1' || event.code === 'KeyZ') && MortyX - 30 > -40 && MortyY + 30 < 470) {
      setRight(false);
      setup(false);
      setMortyX((prev) => prev - 30);
      setMortyY((prev) => prev + 30);
    }
    if ((event.code === 'Numpad3' || event.code === 'KeyC') && MortyX + 30 < 800 && MortyY + 30 < 470) {
      setRight(true);
      setup(false);
      setMortyX((prev) => prev + 30);
      setMortyY((prev) => prev + 30);
